...
while True:
    ...
    if choice == '-1':
        ...
        #### 프로그램 구현
        sql = f'''select a.emp_name,a.department_id, c.department_name, b.emp_name from employees a, 
                employees b, departments c where a.department_id = b.department_id and 
                a.emp_name = '{search}' and a.department_id = c.department_id
                '''
        conn,cursor = connection()
        cursor.execute(sql)
        data = cursor.fetchall()
